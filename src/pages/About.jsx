  ...
  return (
    <main style={{ color: '#fff', backgroundColor: '#070C1A' }}>
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '3.2rem 1.5rem' }}>
        <motion.div {...fromLeft}>
          <h2 style={{ margin: 0, fontFamily: "'Cormorant Garamond', serif", fontSize: '2.4rem', fontWeight: 500 }}>
            Our Company Story
          </h2>
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem 3rem' }}>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {team.map((member, index) => (
            <motion.div key={member.name} {...(index % 2 === 0 ? fromLeft : fromRight)} style={{ border: '1px solid rgba(201,168,76,0.25)', backgroundColor: 'rgba(16,31,72,0.45)', padding: '1rem' }}>
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem 3rem' }}>
        ...
        <div style={{ display: 'grid', gap: '0.7rem' }}>
          {timeline.map((item, index) => (
            <motion.div key={item.year} {...(index % 2 === 0 ? fromLeft : fromRight)} style={{ border: '1px solid rgba(201,168,76,0.2)', backgroundColor: 'rgba(16,31,72,0.35)', padding: '0.8rem 1rem', display: 'grid', gridTemplateColumns: '90px 1fr', gap: '0.8rem' }}>
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem 4rem' }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {certs.map((cert, index) => (
            <motion.div key={cert} {...fromBottom} transition={{ ...transition, delay: index * 0.1 }} style={{ border: '1px solid rgba(201,168,76,0.3)', backgroundColor: 'rgba(7,12,26,0.6)', padding: '0.9rem', textAlign: 'center' }}>
